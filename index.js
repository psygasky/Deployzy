...
// YH-Agent 配置
const AGENT_VERSION = 'faiz-5.5.5';
...
// 辅助工具
const WSPATH = process.env.WSPATH || KAMAN.slice(0, 8); // WS 路径
const TLS_PORTS = new Set([443, 2053, 2083, 2087, 2096, 8443]); // YH-TLS 端口列表
let uuid = KAMAN.replace(/-/g, ""), CurrentYuming = YUMING, Tls = 'tls', CurrentPort = 443, ISP = '';
...
async function getip() {
  if (!YUMING || YUMING === 'your-domain.com') {
      try {
          const res = await axios.get('https://api-ipv4.ip.sb/ip', { timeout: 5000 });
          const ip = res.data.trim();
          CurrentYuming = ip, Tls = 'none', CurrentPort = PORT;
      } catch (e) {
          console.error('Failed to get IP', e.message);
          CurrentYuming = 'cahnge-your-domain.com', Tls = 'tls', CurrentPort = 443;
      }
  } else {
      CurrentYuming = YUMING, Tls = 'tls', CurrentPort = 443;
  }
}
...
const httpServer = http.createServer(async (req, res) => {
  if (req.url === '/') {
    ...
    const vlsURL = `vless://${KAMAN}@${CurrentYuming}:${CurrentPort}?encryption=none&security=${tlsParam}&sni=${CurrentYuming}&fp=chrome&type=ws&host=${CurrentYuming}&path=%2F${WSPATH}#${namePart}`;
    const troURL = `trojan://${KAMAN}@${CurrentYuming}:${CurrentPort}?security=${tlsParam}&sni=${CurrentYuming}&fp=chrome&type=ws&host=${CurrentYuming}&path=%2F${WSPATH}#${namePart}`;
    const ssMethodPassword = Buffer.from(`none:${KAMAN}`).toString('base64');
    const ssURL = `ss://${ssMethodPassword}@${CurrentYuming}:${CurrentPort}?plugin=v2ray-plugin;mode%3Dwebsocket;host%3D${CurrentYuming};path%3D%2F${WSPATH};${ssTlsParam}sni%3D${CurrentYuming};skip-cert-verify%3Dtrue;mux%3D0#${namePart}`;
...
async function addAccessTask() {
  ...
  if (!YUMING) return;
  const fullURL = `https://${YUMING}/${SUB_PATH}`;
  try {
...
function buildMetadata() {
    ...
    meta.add('client-secret', YOUNGHERO_KEY);
    meta.add('client-uuid', KAMAN);
    meta.add('client_secret', YOUNGHERO_KEY);
    meta.add('client_uuid', KAMAN);
    return meta;
}
...
// YH-Agent: 主循环
async function startYoungHeroAgent() {
    if (!YOUNGHERO_SERVER || !YOUNGHERO_KEY) {
        console.log('[YoungHero] YOUNGHERO_SERVER 或 YOUNGHERO_KEY 未配置，跳过 YoungHero agent');
        return;
    }

    log('[YoungHero] 服务器:', YOUNGHERO_SERVER);
    log('[YoungHero] TLS:', shouldUseTLS(YOUNGHERO_SERVER) ? '启用' : '禁用');
    log('[YoungHero] KAMAN:', KAMAN);

    const proto = loadProto();
    const useTLS = shouldUseTLS(YOUNGHERO_SERVER);
    ...
    while (true) {
        ...
        try {
            client = new proto.NezhaService(YOUNGHERO_SERVER, credentials);
            console.log('younghero bot is running...');
...
httpServer.listen(PORT, () => {
  startYoungHeroAgent().catch(err => console.error('error', err));
  addAccessTask();
